...
6. When **`ě`** follows a dental/alveolar consonant (**`t` `d` `s` `z` `n` `l` `r`**), this consonant is pronounced soft (see [phonology][1]). After other consonants it produces the same sound as **`e`**, but preceded by **`j`**: _`dělo`_ sounds like _\*`ďelo`_, _`pěsok`_ like _\*`pjesok`_.
