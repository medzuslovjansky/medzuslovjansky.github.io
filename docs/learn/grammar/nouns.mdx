...
- The locative singular is where the Slavic languages differ most. The recommended ending is `-u` (i.e. the same as the dative), which in most languages occurs at least in some instances. Alternatively, it is also possible to use `-ě` after hard consonants and `-i` after soft consonants (e.g. :isv[bratě], _`muži`_).
- In the vocative, `k`, `g` and `h` become `č`, `ž` and `š` before `e`: :isv[člověk] > :isv[člověče], _`Bog`_ > _`Bože`_.
- Words on `-ec` have the vocative ending `-če` instead of the expected `-cu`: :isv[otec] > _`otče`_.
...
- There are a few words on `-a` denoting male persons, such as :isv[sluga] „servant”, :isv[kolega] „colleague”, :isv[sudja] „judge”, etc. In the plural these are inflected like :isv[brat] or _`muž`_, but in the singular, they follow the pattern of the [second declension][2].
...
- a hard consonant, then the root is `-et-`: :isv[tele], gen. _`teleta`_ (noun of this group usually refer to children or young animals)
- a soft consonant, then the root is `-Ø-`: :isv[morje], pl. :isv[morja] (in other words, an `-o` noun affected by the o/e rule)
...
The `-a` declension also includes a number of masculine nouns with the ending `-a` referring to male persons, like `sluga` „servant” and `sudja` „judge”. They are inflected like :isv[žena] or :isv[zemja] in the singular, but in the plural they follow the pattern of masculine animate nouns: :abbr[nom.sg.] :isv[sluga], :abbr[gen.sg.] :isv[slugy], :abbr[nom.pl.] :isv[slugi], :abbr[gen.pl.] _`slugov`_, etc.
...
- masculine nouns on `-enj`, a tiny group including e.g. :isv[kamenj] „stone” and _`korenj`_ „root”
- neuter nouns on -`me`/-`men`-, containing numerous nouns like _`ime`_ „name”, :isv[rame] „shoulder”, _`brěme`_ „burden”
- neuter nouns on -`e`/-`et`-, referring to children and young animals, e.g. :isv[tele] „calf”, _`prase`_ „piglet”
- neuter nouns on -`o`/-`es`-, a group of only a few words, e.g. _`nebo`_ „heaven”
- feminine nouns on `-ov`, e.g. :isv[crkov] „church”, _`mrkov`_ „carrot”
- feminine nouns on **-`i`/-`er`-**, containing only two nouns: :isv[mati] „mother” and _`doči`_ „daughter”
...
- `kamenj` is inflected like an ordinary masculine noun (gen. :isv[kamenja], dat. _`kamenju`_)
- `ime` and `tele` are inflected like :isv[slovo], as if their nominatives were `imeno` and `teleto`: gen. _`imena`, `teleta`_, dat. _`imenu`, `teletu`_
- `nebo` is inflected like :isv[slovo] too (gen. :isv[neba], dat. _`nebu`_)
- `crkov` is inflected like :isv[kost] (gen. :isv[crkvi]); instead of `crkov`, it is also possible to use the form `crkva`, which is inflected lke :isv[žena] (gen. :isv[crkvy], dat. _`crkvě`_)
- `mati` is inflected like :isv[kost]: gen./dat. _`materi`_
