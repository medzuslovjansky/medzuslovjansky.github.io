...
- :isv[ime] and `tele` are inflected like :isv[slovo], as if their nominatives were :isv[imeno] and :isv[teleto]: :abbr[gen.] :isv[imena], :isv[teleta], :abbr[dat.] :isv[imenu], :isv[teletu]
- :isv[nebo] is inflected like :isv[slovo] too (:abbr[gen.] :isv[neba], :abbr[dat.] :isv[nebu])
- :isv[crkov] is inflected like :isv[kost] (:abbr[gen.] :isv[crkvi]); instead of `crkov`, it is also possible to use the form :isv[crkva], which is inflected lke :isv[žena] (:abbr[gen.] :isv[crkvy], :abbr[dat.] :isv[crkvě])
- :isv[mati] is inflected like :isv[kost]: :abbr[gen./dat]. :isv[materi]
...
Borrowed international vocabulary ending in `-e`, `-i` or `-u` (e.g. :abbr[alibi], :abbr[hobi], :abbr[intervju], :abbr[kafe], :abbr[kakao], :abbr[kliše], :abbr[menju], :abbr[tabu], :abbr[taksi]) and abbreviations are indeclinable. It is not impossible to add case endings anyway, but in that case it is best to separate them from the noun with an apostrophe: :abbr[togo alibi’a], :abbr[te intervju’y], :abbr[tyh taksi’ov] itd.
