...
Basically, there are two different methods for dealing with the past tense. Most commonly used is the compound past, made by combining the present tense of the verb :isv[byti] „to be” with the so-called _`L-participle`_. This participle carries a meaning similar to „being in a state of having done something”, and is formed by adding the ending `-l` (masculine singular), `-la` (feminine singular), `-lo` (neuter singular) or `-li` (universal plural) to the infinitive stem.
...
The verbs :isv[dati] „to give”, :isv[jesti] „to eat” and :isv[věděti] „to know” have an irregular present tense. Like :isv[byti], they belong to the so-called _`athematic conjugation`_, meaning that they have their root ending in a vowel. In the present tense, they are inflected as follows:
