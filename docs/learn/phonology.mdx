...
They play a crucial role in both inflection and the world building process, and thus cannot be avoided even in the most simplified form of Interslavic—at least, if we want to avoid forms that come across as heavily artificial and unnatural.
...
Thus: _sg._ :isv[Čeh] „Czech” > _pl._ :isv[Čehi] „Czechs”; _sg._ :isv[dȯlgy] „long” > _pl._ :isv[dȯlgi] (animate) or :isv[dȯlge] (inanimate).
...
For example: _m.pl._ :isv[dom-**y**] versus :isv[kraj-**e**], _f.pl._ :isv[žen-**y**] versus :isv[zemj-**e**].
...
- in the nominative (and, in the case of inanimate nouns, the accusative) singular of masculine nouns, particularly those ending in `-ec` and `-ok`, as well as a few other nouns:
  _nom.sg._ :isv[ot**ė**c] „father” > _gen.sg._ :isv[otca]
  _nom.sg._ :isv[pěs**ȯ**k] „sand” > _gen.sg._ :isv[pěska]
  _nom.sg._ :isv[p**ė**s] „dog” > _gen.sg._ :isv[psa]
  _nom.sg._ :isv[s**ȯ**n] „dream” > _gen.sg._ :isv[sna]
  _nom.sg._ :isv[kr**ȯ**v] „blood” > _gen.sg._ :isv[krvi]
- in the genitive plural of feminine and neuter nouns that would otherwise end in a consonant cluster:
  _nom.sg._ :isv[okno] „window” > _gen.pl._ :isv[ok**ė**n]
  _nom.sg._ :isv[miska] „bowl” > _gen.pl._ :isv[mis**ȯ**k]
- in the nominative singular masculine forms of certain pronouns:
  _m.nom.sg._ :isv[v**ė**ś] „all, entire”, _f.nom.sg._ :isv[vśa]
...
- the prepositions :isv[s] and :isv[v] when preceding certain pronouns or consonant clusters, or when used as a prefix before a vowel or a consonant cluster:
  _`s`_ „with” + :isv[mnojų] „me” (_instr.sg._) > :isv[s**ȯ** mnojų],
  _`v`_ „in” + :isv[vsih] „all” (_loc.pl._) > :isv[v**ȯ** vsih],
