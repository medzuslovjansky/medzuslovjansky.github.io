...
- Первое спряжение:
  ...
  - глаголы на `-ati`, односложные основы которых оканчиваются на гласную, и ряд глаголов на `-ěti` имеют `-j-`, добавленные к основе инфинитива: _`dělati > dělaj-`_, _`uměti > uměj-`_, _`biti > bij-`_, _`čuti > čuj-`_
...
По сути, есть два разных метода работы с прошедшим временем. Чаще всего используется составное прошедшее время, образованное путем соединения настоящего времени глагола `byti` «быть» с так называемым _`L-participle`_. Это причастие имеет значение, аналогичное «находиться в состоянии совершения чего-либо», и образуется путем добавления окончания `-l` (мужской род единственного числа), `-la` (женский род единственного числа), `-lo` (средний род единственного числа) или `-li` (универсальное множественное число) к основе инфинитива.
...
- Глаголы на `-iti` имеют `-jeny` вместо **\*-`iny`** (где **\*-`sjeny`, \*-`zjeny`, \*-`tjeny`, \*- `djeny`, \*-`stjeny`, \*-`zdjeny`** становятся **-`šeny`, -`ženy`, -`čeny`, -`dženy`, -`ščeny`, -`ždženy`**): :isv[govoriti > govorjeny] «разговорный», :isv[ljubiti > ljubjeny] «любимый», _`prositi > prošeny`_ «требуемый»
