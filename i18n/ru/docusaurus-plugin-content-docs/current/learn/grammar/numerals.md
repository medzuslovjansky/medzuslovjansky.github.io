...
* Числа 0-9 показаны в первом столбце: :isv[nula] для нуля и :isv[jedin] – :isv[devet] для 1-9.
* Числа 11-19 образуются путём прибавления :isv[-nadset] (произносится :isv[-nacet]) к числам 1-9, как показано во втором столбце (например, :isv[jedinnadset], :isv[dvanadset]).
* Десятки (10-90) состоят из :isv[deset] для 10, а для 20-90 — из чисел 2-9 с суффиксом :isv[-deset] (например, :isv[dvadeset], :isv[trideset]), как показано в третьем столбце.
* Сотни (100-900) могут образовываться двумя способами, как показано в четвёртом столбце:
  * :isv[sto] для 100, а для 200-900 — прибавлением чисел 2-9 к :isv[-sto] (например, :isv[dvasto], :isv[tristo])
  * Или с помощью альтернативных форм со склоняемыми окончаниями (например, :isv[dvěstě], :isv[trista], :isv[petsot])

Слова для «тысячи», «миллиона» и «миллиарда»: `tyseč` (1000), `milion` (10⁶) и `miliard` (10⁹). Как и в случае сотен, эти слова могут склоняться или не склоняться как существительные. Хотя в славянских языках это не принято, в случаях типа `pet-tyseć` «5000» для ясности может быть добавлен дефис.

Их комбинации всегда составляются от большего к меньшему: тысячи – сотни – десятки – единицы. Между десятками и единицами может быть вставлено слово `i` «и». Например: `tri-tyseč četyristo petdeset (i) šest` «3,456». Больше примеров можно увидеть в конвертере ниже.
