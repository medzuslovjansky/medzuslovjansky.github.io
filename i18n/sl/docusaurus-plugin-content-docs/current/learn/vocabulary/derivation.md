...
- Izogibamo se geminiranim soglasnikom: :isv[gramofon], :isv[grupa].
- Grška soglasnika `th` in `ph` sta poenostavljena v `t` in `f`: :isv[teatr], :isv[fenomen].
- Grško `y` se piše `i`, kar je bližje tako črkovanju kot izgovorjavi v večini slovanskih jezikov: :isv[sistem], :isv[fizika].
- Zvok :ipa[k] je vedno zapisan `k` in nikoli `c`: :isv[kontakt].
- Zvok :ipa[x] (pogosto predstavljen z `ch`) je zapisan `h`: :isv[psiholog].
- Zvok :ipa[y] (v nemščini napisano `ü`, v francoščini `u`) postane `ju`: :isv[bjuro].
- Med samoglasniki zvok :ipa[z] (zapisan `-s-`) postane `z`: :isv[baza].
- Med samoglasniki zvok :ipa[s] (zapisan `-ss-`) postane `s`: :isv[masa].

Poleg tega so končnice na predvidljiv način prilagojene temu, kar je najpogostejše v slovanskih jezikih:

- Glagoli latinskega izvora (pogosto `-ate`, `-fy`, `-ise`/`-ize` v angleščini, `-ieren` v nemščini) imajo `-ovati`: :isv[organizovati], :isv[komunikovati].
- Latinski `-ia` (v angleščini preveden kot `-ia` ali `-y`) postane `-ija`: :isv[ekonomija].
- Angleščina `-sis` (iz grščine) postane `-za`: :isv[kriza].
- Imena kemijskih elementov itd. na `-ium` postanejo `-ij`: :isv[helij], :isv[kriterij].
- Besede na `-um` in `-us` običajno ohranijo te končnice: :isv[forum], :isv[korpus].
- Angleščina `-ty` (latinica `-tas`, `-tatis`) postane `-tet`: :isv[universitet].
- Angleščina `-ics` postane `-ika`: :isv[ekonomika].
- Angleščina `-ism` postane `-izm`: :isv[komunizm].
- Angleščina `-ist` postane `-ist`: :isv[komunist].
- Angleščina `-ssion` postane `-sija`: :isv[diskusija].
- Angleščina `-sion` postane `-zija`: :isv[televizija].
- Angleščina `-tion` postane `-cija`: :isv[akcija].
- Pridevniki, izpeljani iz teh samostalnikov, imajo končnico `-ijny`: :isv[televizijny], :isv[tradicijny].
- Angleški pridevniki v `-al` (latinsko `-alis`) se končajo na `-alny`: :isv[neutralny].
- Angleški pridevniki v `-ic`, `-ical` (latinsko `-icus`) imajo končnico `-ičny`: :isv[specifičny], :isv[komičny].
- Angleški pridevniki v `-ive` (latinsko `-ivus`) imajo končnico `-ivny`: :isv[pozitivny].
- Angleški pridevniki v `-ous` (latinsko `-osus`) imajo končnico `-ozny`: :isv[seriozny].

Bolj problematične so besede, prevzete iz angleščine. Pri pisanju v latinici lahko ohranimo prvotni zapis (_bypass_, _knockout_, _jazz_, _teenager_). Veliko besedil v medslovanščini pa je napisanih v cirilici ali v obeh pravopisih vzporedno, pogosto s pomočjo programa za prečrkovanje, zato se je seveda treba izogibati pisanju _быпасс_ ali _кноцкоут_ v cirilici! V večini primerov bi zato raje uporabili bolj fonetični pristop: :isv[bajpas], :isv[nokaut], :isv[džaz], :isv[tinejdžer], :isv[budžet], :isv[biznes], :isv[mjuzikl], :isv[futbol], :isv[koktejl] itd.
