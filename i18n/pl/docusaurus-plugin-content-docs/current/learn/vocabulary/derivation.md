...
- Unika się bliźniaczych spółgłosek: :isv[gramofon], :isv[grupa].
- Greckie spółgłoski `th` i `ph` są uproszczone do `t` i `f`: :isv[teatr], :isv[fenomen].
- Grecki `y` jest zapisywany jako `i`, co jest bliższe pisowni i wymowie większości języków słowiańskich: :isv[sistem], :isv[fizika].
- Dźwięk :ipa[k] jest zawsze zapisywany `k` i nigdy `c`: :isv[kontakt].
- Dźwięk :ipa[x] (często reprezentowany przez `ch`) jest zapisywany jako `h`: :isv[psiholog].
- Dźwięk :ipa[y] (w niemieckim zapisany jako `ü`, we francuskim `u`) staje się `ju`: :isv[bjuro].
- Między samogłoskami dźwięk :ipa[z] (pisany jako `-s-`) staje się `z`: :isv[baza].
- Między samogłoskami dźwięk :ipa[s] (pisany jako `-ss-`) staje się `s`: :isv[masa].

Poza tym końcówki dostosowane są w przewidywalny sposób do tego, co najczęściej spotyka się w językach słowiańskich:

- Czasowniki pochodzenia łacińskiego (często `-ate`, `-fy`, `-ise`/`-ize` w języku angielskim, `-ieren` w języku niemieckim) mają `-ovati`: :isv[organizovati], :isv[komunikovati].
- Łacińskie `-ia` staje się `-ija`: :isv[ekonomija].
- Angielskie `-sis` (z greckiego) staje się `-za`: :isv[kriza].
- Nazwy pierwiastków chemicznych itp. kończące się na `-ium` stają się `-ij`: :isv[helij], :isv[kriterij].
- Słowa kończące się na `-um` i `-us` generalnie zachowują następujące zakończenia: :isv[forum], :isv[korpus].
- Angielskie `-ty` (łac. `-tas`, `-tatis`) staje się `-tet`: :isv[universitet].
- Angielski `-ics` staje się `-ika`: :isv[ekonomika].
- Angielski `-ism` staje się `-izm`: :isv[komunizm].
- Angielski `-ist` staje się `-ist`: :isv[komunist].
- Angielski `-ssion` staje się `-sija`: :isv[diskusija].
- Angielski `-sion` staje się `-zija`: :isv[televizija].
- Angielski `-tion` staje się `-cija`: :isv[akcija].
- Przymiotniki pochodzące od tych rzeczowników mają końcówkę `-ijny`: :isv[televizijny], :isv[tradicijny].
- Angielskie przymiotniki z `-al` (łac. `-alis`) kończą się na `-alny`: :isv[neutralny].
- Angielskie przymiotniki z `-ic`, `-ical` (łac. `-icus`) mają końcówkę `-ičny`: :isv[specifičny], :isv[komičny].
- Angielskie przymiotniki z `-ive` (łac. `-ivus`) mają końcówkę `-ivny`: :isv[pozitivny].
- Angielskie przymiotniki z `-ous` (łac. `-osus`) mają końcówkę `-ozny`: :isv[seriozny].

Bardziej problematyczne są słowa zapożyczone z języka angielskiego. Pisząc alfabetem łacińskim możemy zachować oryginalną pisownię (_bypass_, _knockout_, _jazz_, _teenager_). Jednak wiele tekstów w języku starosłowiańskim jest pisanych cyrylicą lub w obu ortografiach równolegle, często z pomocą programu do transliteracji, i oczywiście należy unikać pisania _быпасс_ lub _кноцкоут_ w cyrylicy! W większości przypadków wolelibyśmy zatem raczej zastosować podejście fonetyczne: :isv[bajpas], :isv[nokaut], :isv[džaz], :isv[tinejdžer], :isv[budžet], :isv[biznes], :isv[mjuzikl], :isv[futbol], :isv[koktejl] itd.
