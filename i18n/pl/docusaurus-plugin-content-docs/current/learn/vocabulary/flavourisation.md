...
Litera `å` nie występuje w żadnym języku słowiańskim jako odrębny fonem. Intersłowiański używa `a` w tej pozycji, podobnie jak staro-cerkiewno-słowiański, południowosłowiański, czeski, słowacki i wiele słów w języku rosyjskim. Aby nadać tekstowi bardziej północnosłowiański charakter, zamiast tego można użyć `o`.
...
Standardowa cyrylica jest oparta na cyrylicy serbsko-macedońskiej, z dodatkiem `ы` (z rosyjskiego i białoruskiego) oraz `є` (z ukraińskiego). Jest to o tyle problematyczne, że Słowianie wschodni nie zawsze poprawnie rozumieją cyrylicę `ј`, którą zwykle wymawiają jako `dž`, jak w języku angielskim, podczas gdy wielu Słowian południowych nie zna litery `ы`. W sytuacjach, gdy tekst jest prezentowany zarówno po łacinie, jak i cyrylicy, dobrym pomysłem może być użycie doprawionej pisowni, która jest specjalnie skierowana do Słowian wschodnich, ponieważ Serbowie i Macedończycy są bardziej zaznajomieni z alfabetem łacińskim.
