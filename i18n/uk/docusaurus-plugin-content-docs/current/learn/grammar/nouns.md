...
Оскільки ми зазвичай не розрізняємо `ę` і `e`, корисно пам’ятати, що іменник із закінченням на :isv[-e] завжди є середнім родом, і, як правило, коли цьому :isv[-e] передує:
