...
* Числа 0-9 показані в першому стовпці: :isv[nula] для нуля та :isv[jedin] – :isv[devet] для 1-9.
* Числа 11-19 утворюються додаванням :isv[-nadset] (вимовляється :isv[-nacet]) до чисел 1-9, як показано в другому стовпці (наприклад, :isv[jedinnadset], :isv[dvanadset]).
* Десятки (10-90) складаються з :isv[deset] для 10, а для 20-90 — з чисел 2-9 із суфіксом :isv[-deset] (наприклад, :isv[dvadeset], :isv[trideset]), як показано в третьому стовпці.
* Сотні (100-900) можна утворити двома способами, як показано в четвертому стовпці:
  * :isv[sto] для 100, а для 200-900 — додаванням чисел 2-9 до :isv[-sto] (наприклад, :isv[dvasto], :isv[tristo])
  * Або за допомогою альтернативних форм із відмінюваними закінченнями (наприклад, :isv[dvěstě], :isv[trista], :isv[petsot])

Слова для «тисячі», «мільйона» та «мільярда» такі: :isv[tyseč] (1000), :isv[milion] (10⁶) та :isv[miliard] (10⁹). Як і у випадку з сотнями, ці слова можуть відмінюватися або не відмінюватися як іменники. Хоча в слов’янських мовах це не прийнято робити, у таких випадках, як :isv[pet-tyseć] „5000” можна додати дефіс для ясності.

Ці комбінації завжди складаються від великого до низького: тисячі – сотні – десятки – одиниці. Між десятками та одиницями можна вставити слово `i` „і”. Наприклад: :isv[tri-tyseč četyristo petdeset (i) šest] „3,456”. Більше прикладів можна побачити в конвертері нижче.
