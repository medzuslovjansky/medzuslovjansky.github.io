...
- Прва конјугација:
  ...
  - глаголи на `-ati`, едносложни стебла што завршуваат на самогласка, и голем број на глаголи на `-ěti` имаат додадено `-j-` на инфинитивното стебло: _`dělati > dělaj-`_, _`uměti > uměj-`_, _`biti > bij-`_, _`čuti > čuj-`_
...
Во основа, постојат два различни методи за справување со минато време. Најчесто се користи сложеното минато, направено со комбинирање на сегашното време од глаголот `byti` „да се“ со т.н. _`L-participle`_. Овој партицип има значење слично на „да се биде во состојба да има направено нешто“ и се формира со додавање на завршетокот `-l` (машки род во еднина), `-la` (женска еднина), `-lo` (неутрална еднина) или `-li` (универзална множина) до инфинитив стебло.
...
- Глаголите на `-iti` имаат `-jeny` наместо **\*-`iny`** (при што **\*-`sjeny`, \*-`zjeny`, \*-`tjeny`, \*- `djeny`, \*-`stjeny`, \*-`zdjeny`** стануваат **-`šeny`, -`ženy`, -`čeny`, -`dženy`, -`ščeny`, -`ždženy`**): :isv[govoriti > govorjeny] „зборувано“, :isv[ljubiti > ljubjeny] „сакано“, _`prositi > prošeny`_ „задолжително“
