...
- Първо спрежение:
  ...
  - глаголи на `-ati`, едносрични основи, завършващи на гласна, и редица глаголи на `-ěti` имат `-j-`, добавен към инфинитивната основа: _`dělati > dělaj-`_, _`uměti > uměj-`_, _`biti > bij-`_, _`čuti > čuj-`_
...
По принцип има два различни метода за справяне с миналото време. Най-често използваното е съставното минало, направено чрез комбиниране на сегашно време на глагола `byti` „to be” с така нареченото _`L-participle`_. Това причастие носи значение, подобно на „да си в състояние да си направил нещо“ и се формира чрез добавяне на окончание `-l` (мъжки род единствено число), `-la` (женски род единствено число), `-lo` (среден род единствено число) или `-li` (универсално множествено число) към инфинитивната основа.
...
- Глаголите на `-iti` имат `-jeny` вместо **\*-`iny`** (при което **\*-`sjeny`, \*-`zjeny`, \*-`tjeny`, \*- `djeny`, \*-`stjeny`, \*-`zdjeny`** стават **-`šeny`, -`ženy`, -`čeny`, -`dženy`, -`ščeny`, -`ždženy`**): :isv[govoriti > govorjeny] „говорен”, :isv[ljubiti > ljubjeny] „обичан”, _`prositi > prošeny`_ „задължителен”
