...
В допълнение, в инфинитивите за северно ароматизиране, завършващи на `-ť` вместо на `-ti`, именителният/винителен падеж единствено число на прилагателните трябва да бъде `-e` вместо `-o` (:isv[dobre detja] вместо южно :isv[dobro děte]). Северното овкусяване предпочита относителното местоимение :isv[ktory], докато южното овкусяване предпочита :isv[koj]. По същия начин, в случай на префикс `vy-`/`iz-`, първият е ориентиран на север, а вторият - на юг. В южната ароматизация предлогът :isv[dlja] „за” трябва да се замени с :isv[za].
