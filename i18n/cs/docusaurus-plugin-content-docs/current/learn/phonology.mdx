...
Tedy: _sg._ :isv[Čeh] „Čech“ > _pl._ :isv[Čehi] „Češi“; _sg._ :isv[dȯlgy] „dlouhý” > _pl._ :isv[dȯlgi] (živý) nebo :isv[dȯlge] (neživý).
...
Například: _m.pl._ :isv[dom-**y**] versus :isv[kraj-**e**], _f.pl._ :isv[žen-**y**] versus :isv[zemj-**e**].
...
- v nominativu (a v případě neživotných podstatných jmen v akuzativu) jednotném čísle podstatných jmen mužského rodu, zejména těch končících na `-ec` a `-ok`, jakož i několika dalších podstatných jmen:
   _nom.sg._ :isv[ot**ė**c] „otec“ > _gen.sg._ :isv[otca]
   _nom.sg._ :isv[pěs**ȯ**k] „písek” > _gen.sg._ :isv[pěska]
   _nom.sg._ :isv[p**ė**s] „pes“ > _gen.sg._ :isv[psa]
   _nom.sg._ :isv[s**ȯ**n] „sen“ > _gen.sg._ :isv[sna]
   _nom.sg._ :isv[kr**ȯ**v] „krev“ > _gen.sg._ :isv[krvi]
...
- v nominativu jednotného čísla mužského rodu určitých zájmen:
   _m.nom.sg._ :isv[v**ė**ś] „vše, celé“, _f.nom.sg._ :isv[vśa]
...
- předložky :isv[s] a :isv[v] před určitými zájmeny nebo shluky souhlásek nebo při použití jako předpona před samohláskou nebo shlukem souhlásek:
   _`s`_ „s“ + :isv[mnojų] „já“ (_instr.sg._) > :isv[s**ȯ** mnojų],
   _`v`_ „in“ + :isv[vsih] „vše“ (_loc.pl._) > :isv[v**ȯ** vsih],
