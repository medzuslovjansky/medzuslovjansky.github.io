...
- Grčki suglasnici `th` i `ph` pojednostavljeni su u `t` i `f`: :isv[teatr], :isv[fenomen].
- Grčki `y` piše se `i`, što je bliže i pravopisu i izgovoru u većini slavenskih jezika: :isv[sistem], :isv[fizika].
- Zvuk :ipa[k] uvijek se piše `k`, a nikad `c`: :isv[kontakt].
- Zvuk :ipa[x] (često predstavljen sa `ch`) piše se `h`: :isv[psiholog].
- Zvuk :ipa[y] (na njemačkom napisano `ü`, na francuskom `u`) postaje `ju`: :isv[bjuro].
- Između samoglasnika, glas :ipa[z] (napisan `-s-`) postaje `z`: :isv[baza].
- Između samoglasnika, glas :ipa[s] (napisan `-ss-`) postaje `s`: :isv[masa].

Osim toga, završeci su na predvidljiv način prilagođeni onome što je najčešće u slavenskim jezicima:

- Glagoli latinskog porijekla (često `-ate`, `-fy`, `-ise`/`-ize` u engleskom, `-ieren` u njemačkom) imaju `-ovati`: :isv[organizovati], :isv[komunikovati].
- Latinski `-ia` (preveden na engleski kao `-ia` ili `-y`) postaje `-ija`: :isv[ekonomija].
- Engleski `-sis` (od grčkog) postaje `-za`: :isv[kriza].
- Nazivi kemijskih elemenata itd. na `-ium` postaju `-ij`: :isv[helij], :isv[kriterij].
- Riječi na `-um` i `-us` općenito imaju ove završetke: :isv[forum], :isv[korpus].
...
- Engleski pridjevi u `-al` (latinski `-alis`) završavaju na `-alny`: :isv[neutralny].
- Engleski pridjevi na `-ic`, `-ical` (latinski `-icus`) imaju završetak `-ičny`: :isv[specifičny], :isv[komičny].
- Engleski pridjevi u `-ive` (latinski `-ivus`) imaju završetak `-ivny`: :isv[pozitivny].
- Engleski pridjevi u `-ous` (latinski `-osus`) imaju završetak `-ozny`: :isv[seriozny].

Problematičnije su riječi posuđenice iz engleskog. Kod pisanja latiničnim pismom možemo zadržati izvorni način pisanja (_bypass_, _knockout_, _jazz_, _teenager_). Međutim, mnogi su tekstovi na međuslavenskom pisani ćirilicom ili u oba pravopisa paralelno, često uz pomoć programa za transliteraciju, te naravno treba izbjegavati pisanje _быпасс_ ili _кноцкоут_ ćirilicom! U većini slučajeva radije bismo primijenili fonetski pristup: :isv[bajpas], :isv[nokaut], :isv[džaz], :isv[tinejdžer], :isv[budžet], :isv[biznes], :isv[mjuzikl], :isv[futbol], :isv[koktejl] itd.
