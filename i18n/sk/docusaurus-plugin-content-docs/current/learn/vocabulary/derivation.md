...
- Vyhýbajú sa dvojhlásky: :isv[gramofon], :isv[grupa].
- Grécke spoluhlásky `th` a `ph` sú zjednodušené na `t` a `f`: :isv[teatr], :isv[fenomen].
- Grécky `y` sa píše `i`, čo je bližšie k pravopisu aj výslovnosti vo väčšine slovanských jazykov: :isv[sistem], :isv[fizika].
- Zvuk :ipa[k] sa vždy píše `k` a nikdy nie `c`: :isv[kontakt].
- Zvuk :ipa[x] (často reprezentovaný `ch`) sa píše `h`: :isv[psiholog].
- Zvuk :ipa[y] (v nemčine napísané `ü`, vo francúzštine `u`) sa zmení na `ju`: :isv[bjuro].
- Medzi samohláskami sa zvuk :ipa[z] (napísaný `-s-`) zmení na `z`: :isv[baza].
- Medzi samohláskami sa zvuk :ipa[s] (napísaný `-ss-`) zmení na `s`: :isv[masa].

Okrem toho sú koncovky prispôsobené predvídateľným spôsobom tomu, čo je najbežnejšie v slovanských jazykoch:

- Slovesá latinského pôvodu (často `-ate`, `-fy`, `-ise`/`-ize` v angličtine, `-ieren` v nemčine) majú `-ovati`: :isv[organizovati], :isv[komunikovati].
- Latinské `-ia` (prevedené v angličtine ako `-ia` alebo `-y`) sa zmení na `-ija`: :isv[ekonomija].
- anglické `-sis` (z gréčtiny) sa zmení na `-za`: :isv[kriza].
- Názvy chemických prvkov atď. na `-ium` sa stanú `-ij`: :isv[helij], :isv[kriterij].
- Slová na `-um` a `-us` majú vo všeobecnosti tieto koncovky: :isv[forum], :isv[korpus].
- anglický `-ty` (latinsky `-tas`, `-tatis`) sa zmení na `-tet`: :isv[universitet].
- anglické `-ics` sa zmení na `-ika`: :isv[ekonomika].
- anglické `-ism` sa zmení na `-izm`: :isv[komunizm].
- anglické `-ist` sa zmení na `-ist`: :isv[komunist].
- anglické `-ssion` sa zmení na `-sija`: :isv[diskusija].
- anglické `-sion` sa zmení na `-zija`: :isv[televizija].
- anglické `-tion` sa zmení na `-cija`: :isv[akcija].
- Prídavné mená odvodené od týchto podstatných mien majú koncovku `-ijny`: :isv[televizijny], :isv[tradicijny].
- Anglické prídavné mená v `-al` (latinka `-alis`) končia na `-alny`: :isv[neutralny].
- Anglické prídavné mená v `-ic`, `-ical` (latinsky `-icus`) majú koncovku `-ičny`: :isv[specifičny], :isv[komičny].
- Anglické prídavné mená v `-ive` (latinsky `-ivus`) majú koncovku `-ivny`: :isv[pozitivny].
- Anglické prídavné mená v `-ous` (latinka `-osus`) majú koncovku `-ozny`: :isv[seriozny].

Problematickejšie sú slová prevzaté z angličtiny. Pri písaní latinkou môžeme ponechať pôvodný pravopis (_bypass_, _knockout_, _jazz_, _teenager_). Mnohé texty v interslovančine sú však písané v azbuke alebo v oboch pravopisoch súbežne, často pomocou prepisovacieho programu, a samozrejme sa treba vyhnúť písaniu _быпасс_ alebo _кноцкоут_ v azbuke! Vo väčšine prípadov by sme preto radšej použili fonetickejší prístup: :isv[bajpas], :isv[nokaut], :isv[džaz], :isv[tinejdžer], :isv[budžet], :isv[biznes], :isv[mjuzikl], :isv[futbol], :isv[koktejl] atď.
